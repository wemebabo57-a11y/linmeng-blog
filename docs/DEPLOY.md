...
站点源码按版本号归档在 `src/` 目录，**每个版本一个文件夹**，历史版本也在其中，可自行选择。每个版本都是一个**完整、自包含**的站点根目录。

| 需求 | 选择 |
|------|------|
| 全新搭建 | 进 [`src/`](../src/) 选版本号最大的那个 |
| 下载 zip | [Releases](https://github.com/wemebabo57-a11y/linmeng-blog/releases) 页面的 `linmeng-blog-vX.Y.Z.zip` |
| 找回旧版本 | 进 [`src/`](../src/) 自行选择对应版本号 |
...
❌ 常见错误：上传成了 `网站根目录/<版本号>/index.php`。这样访问域名会 404，把版本号这一层去掉即可。
